    ...
    public bool ProcessGuiInput(InputEvent inputEvent, Rect2 canvasRect)
    {
        switch (inputEvent)
        {
            case InputEventMouseButton mb:
                ...
                if (scrollVec != Vector2.Zero && mb.IsPressed())
                {
                    if (CurrentControlScheme == ControlScheme.ScrollPans)
                    {
                        if (mb.IsCommandOrControlPressed())
                        {
                            ...
                            var zoom = scrollVec.X + scrollVec.Y > 0 ? 1.0f / _scrollZoomFactor : _scrollZoomFactor;
                            _zoomCallback(zoom, mb.Position);
                            return true;
                        }

                        var panning = scrollVec * mb.Factor;
            ...
            case InputEventMagnifyGesture magnifyGesture:
                // Zoom gesture
                _zoomCallback(magnifyGesture.Factor, magnifyGesture.Position);
                return true;
            case InputEventPanGesture panGesture:
                _panCallback(-panGesture.Delta * ScrollSpeed);
                break;
            case InputEventScreenDrag screenDrag:
                ...
                // else
                // {
                _panCallback(screenDrag.Relative);
                // }
                break;
